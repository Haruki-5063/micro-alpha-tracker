...
"""
    ...
    # 🔄 503混雑を回避するための自動再試行（最大3回リトライ）
    max_retries = 3
    base_delay = 4  # 失敗したら4秒, 8秒, 16秒と待機時間を伸ばす
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # エラーメッセージに503やUnavailableが含まれる場合はリトライ
            err_str = str(e)
            if ("503" in err_str or "UNAVAILABLE" in err_str.upper()) and attempt < max_retries:
                delay = base_delay * (2 ** attempt)
                print(f" ⚠️ Gemini APIが混雑しています (503)。 {delay}秒後に自動リトライします... (試行 {attempt + 1}/{max_retries})")
                time.sleep(delay)
            else:
                print(f" 🚨 Geminiパース確定エラー ({ticker}): {e}")
                return default_res
